...
void main() {
  group(
    'issue #573: windows ICO embeds multiple sizes for shell rescaling',
    () {
      test(
        'generated .ico contains more than one image (multi-size pyramid)',
        () async {
      ...
      test(
        'icon_size cap reduces the embedded set (icon_size: 64 → 5 sizes)',
        () async {
          final src = Image(width: 256, height: 256);
          for (final px in src) {
            px.setRgba(0x10, 0x20, 0x30, 0xFF);
