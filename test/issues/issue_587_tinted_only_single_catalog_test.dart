...
void main() {
  group('issue #587: tinted/dark variants share the single AppIcon catalog',
      () {
    test('tinted-only generates appearances inside same catalog', () {
      final json =
          ios.generateContentsFileAsString('Icon-App', null, 'Icon-App-Tinted');
      ...
      final tinted = images.where((img) {
        ...
        return appearances.any((a) =>
            (a as Map)['appearance'] == 'luminosity' && a['value'] == 'tinted');
      });
    ...
    test('dark-only generates appearances inside same catalog', () {
      final json =
          ios.generateContentsFileAsString('Icon-App', 'Icon-App-Dark', null);
      final decoded = jsonDecode(json) as Map<String, dynamic>;
      final images = (decoded['images'] as List).cast<Map<String, dynamic>>();
      ...
      final dark = images.where((img) {
        ...
        return appearances.any((a) =>
            (a as Map)['appearance'] == 'luminosity' && a['value'] == 'dark');
      });
