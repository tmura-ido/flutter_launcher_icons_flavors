# Migrating to `flutter_launcher_icons_flavors` 0.15

Audience: existing `flutter_launcher_icons` users on 0.13.x or 0.14.x.

## TL;DR checklist

- [ ] Replace `flutter_launcher_icons` with `flutter_launcher_icons_flavors: ^0.15.0` in `dev_dependencies`.
- [ ] Update CLI invocations: `dart run flutter_launcher_icons` → `dart run flutter_launcher_icons_flavors generate`.
...
- [ ] If you have multiple legacy `flutter_launcher_icons-<flavor>.yaml` files, run `dart run flutter_launcher_icons_flavors migrate` to consolidate them.
...
+ dart run flutter_launcher_icons_flavors generate -f my_config.yaml
```

A plain `dart run flutter_launcher_icons_flavors` (no subcommand, no flags) still defaults to `generate`, but explicit subcommands are recommended in scripts and CI.

Search-and-replace targets in your codebase:

- Shell scripts / CI YAML: `flutter_launcher_icons` (the package binary name).
- Dart imports (rare; usually only test code): `package:flutter_launcher_icons/...` → `package:flutter_launcher_icons_flavors/...`.
...
| # | Change | Before | After |
| --- | --- | --- | --- |
| 1 | Package name | `flutter_launcher_icons` | `flutter_launcher_icons_flavors` |
| 2 | CLI canonical form | `dart run flutter_launcher_icons` | `dart run flutter_launcher_icons_flavors generate` |
...
- **`doctor` command.** `dart run flutter_launcher_icons_flavors doctor` reports Dart/Flutter versions, detected configs, source resolution, and any conflicts.
...
```shell
dart run flutter_launcher_icons_flavors doctor
...
If anything is unexpected, file an issue at [`tmura-ido/flutter_launcher_icons_flavors/issues`](https://github.com/tmura-ido/flutter_launcher_icons_flavors/issues).
